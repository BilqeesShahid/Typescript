...
export async function selectEventPrompt(availableEvents: any[]) {
...
export async function ticketQuantityPrompt(ticketStock: number) {
...
export async function selectSeatsPrompt(Seats: number[], Quantity: number): Promise<number> {
    ...
    const { selectedSeats } = await inquirer.prompt({
        ...
        choices: choices,
        validate: (input: any) => input.length === Quantity ? true : `Please select exactly ${Quantity} seats.`,
    

    });
    return selectedSeats;
}
export async function confirmPurchasePrompt(selectedEvent: any, selectedSeats: number[]) {
...
export async function paymentPrompt() :Promise<PaymentDetails>{
